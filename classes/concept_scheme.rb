...
class Scheme
  ...
  def to_rails
    ...
    @domains.each do |d|
      ...
      sql += "Relationship.create([subject_id: (Resource.find_by literal: '#{@id}').id, predicate_id: (Archetype.find_by name: 'domain').id, object_id: (Resource.find_by literal: '#{tid}').id])\n"
    end
    @top_concepts.each do |c|
      tid = c.split(/\=/).last
